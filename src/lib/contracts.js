...
import {rinkeby as addresses} from './addresses.json';
...
export function InitContracts() {
    ...
    const contracts = {
        DerivativeFactory: IDerivativeFactory.attach(addresses.DerivativeFactory).connect(signer),
        IPPool: IIPPool.attach(addresses.IPPool).connect(signer),
        Licenser: ILicenser.attach(addresses.Licenser).connect(signer),
        MockNFT: IMockNFT.attach(addresses.MockNFT).connect(signer)
    };
